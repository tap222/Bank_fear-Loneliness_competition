setwd("/home/tapas/Documents/bigdata/hack earth competition")
...
corrplot(cor(train[,num_col,with=F]),method = "pie")
...
test[,(names(skew_t)) := lapply(.SD, function(x) log(x + 10)), .SDcols = names(skew_t)]

ggplot(train,aes(dti))+geom_density(fill='lightblue',color='black')
...
xd <- h2o.splitFrame(h2o_train,ratios = 0.8)
